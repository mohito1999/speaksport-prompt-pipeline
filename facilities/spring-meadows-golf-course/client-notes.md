...
- Spring Meadows Golf Course is in Gray, Maine and uses facility-local Eastern
...
- The inclusive advance-booking window is 14 calendar days. When the local
  current date is July 27, August 10 is eligible and August 11 is not.
...
- Golf carts are unavailable within two hours of sunset; tee times in that
  window are walking only. Keep this in the receptionist prompt and booking
  workflow, not the booking eligibility backoffice policy.
- Do not guess sunset or cart-cutoff times. When the restriction is known to
  apply, obtain agreement to walk and pass `riding: false`. If a cart is
  required, offer an earlier tee time.
- Preserve and pass the exact course value returned with the selected
  availability result when booking.
- Spring Meadows uses Club Prophet, not ForeUp. Booking references are numeric,
